	...
	: engine(engine), vulkanApi(vulkanApi)
{
    ...
    textureDescriptors.reserve(AVulkan::VulkanConfiguration::maxFramesInFlight);
    for (uint32_t i = 0; i < AVulkan::VulkanConfiguration::maxFramesInFlight; ++i)
...
void VulkanTextureView::Update()
{
    ...
    frame = (frame + 1) % AVulkan::VulkanConfiguration::maxFramesInFlight;
}
